...
(function(Spine, $){  
  ...
  Route.extend({
	...
	// Prasad: To support ability when "catch all" is defined first and
	// specific ones defined later.
	prepend: function(path, callback) {	
	  if (typeof path === "object" && !(path instanceof RegExp))
	    for(var p in path) this.prepend(p, path[p]);
	  else
		this.routes.unshift(this.init(path, callback));
	},
